...
```{r}
knn_clf <- train_model("knn", tuneLength = 5)
```
...
plot(down_knn_clf)
```
Perform parameter tuning using `tuneLength`.
```{r}
down_knn_clf <- train_model("knn", down_train_set, tuneLength = 10)
```
...
```{r}
up_knn_clf <- train_model("knn", up_train_set, tuneLength = 5)
```
