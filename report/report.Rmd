...
####4. draw the Heat map
```{r}
heatmap <- ggplot(data = heat_map_data, aes(x=Var1, y=Var2, fill=value)) +
...
```{r}
G3 <- subset(heat_map_data,Var2=="G3")
G3[3] <- abs(G3[3])
...
```{r}
lr_metrics <- evaluate_model_clf(lr_clf,"LR")
```
...
```{r}
down_lr_metrics <- evaluate_model_clf(down_lr_clf,"LR")
```
...
```{r}
up_lr_metrics <- evaluate_model_clf(up_lr_clf,"LR")
```
